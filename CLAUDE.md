...
| `GET /api/analysis/violations-summary` | `routes/analysis.ts` | Claude AI summary of recent violation themes |
...
**`/api/query` SSE flow:** Claude parses the user's question into a structured `InspectionQuery` using tool use (`build_inspection_query`), the backend fetches matching records from Socrata, then streams a formatted Claude response as SSE events (`meta` → `chunk`... → `done`).
